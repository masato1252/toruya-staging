...
class SocialUserMessage < ApplicationRecord
  ...
  def hi_message
    ...
    if user?
      "💭 `user_id: #{social_user.toruya_user_id}, #{social_user.social_user_name}` #{"<#{Rails.application.routes.url_helpers.admin_chats_url(social_service_user_id: social_user.social_service_user_id, locale: social_user.locale)}|chat link>"}
      #{message_content}"
    end
  end
end
