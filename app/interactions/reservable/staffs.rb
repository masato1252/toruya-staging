...
module Reservable
  class Staffs < ActiveInteraction::Base
    ...
    def execute
      ...
      scoped = menu.staffs.active.where(id: shop.staffs.active.select(:id)).
      ...
      includes(:staff_menus).
        where.not("staff_menus.staff_id" => (reserved_staff_ids(menu) + closed_custom_schedules_staff_ids).uniq)
