...
class Lines::Actions::BookingOptions < ActiveInteraction::Base
      ...
      if booking_page.started? && !booking_page.ended?
        LineMessages::FlexTemplateContent.two_header_card(
          ...
          action_templates: [
            LineActions::Uri.new(
              action: "book",
              url: Rails.application.routes.url_helpers.booking_page_url(booking_page.slug, social_user_id: social_customer.social_user_id, _from: "customer_bot"),
              btn: "primary"
            )
          ].map(&:template)
        )
