...
module Events
  class RegisterParticipant < ActiveInteraction::Base
    ...
    # user_id=2584 is the Toruya official account to save concern data
    TORUYA_OFFICIAL_USER_ID = 2584

    def execute
      participant = event.event_participants.find_or_initialize_by(social_user_id: social_user.id)
      ...
      participant.assign_attributes(
        user_id: social_user.users.first&.id,
      ...
      save_concern_to_toruya_official(participant, social_user, concern_label, concern_category)
    ...
    def save_concern_to_toruya_official(participant, social_user, concern_label, concern_category)
      ...
      social_customer = official_user.social_customers.find_by(social_user_id: social_user.social_service_user_id)
      ...
      Rollbar.error(e, "Failed to save concern to Toruya official customer", social_user_id: social_user.id)
    end
  end
end
