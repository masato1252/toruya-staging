...
const EditEquipment = ({props}) => {
  ...
  const onSubmit = async (data) => {
    ...
    const [error, response] = await CommonServices.update({
      url: Routes.lines_user_bot_settings_shop_equipment_path(props.business_owner_id, props.shop_id, props.equipment.id, {format: "json"}),
      data: {
        equipment: data,
        ...
        <div className="col-sm-6 px-0 settings-view">
          <div className="form with-top-bar">
            <TopNavigationBar
              ...
              title={I18n.t("user_bot.dashboards.settings.equipments.edit_page_title")}
            />

            <div className="field-header">{I18n.t("user_bot.dashboards.settings.equipments.edit_page_header")}</div>
