...
const EventLineLoginLink = ({ loginUrl, btnText, style, captureRegistrationSourceUrl }) => {
  if (!loginUrl) return null;

  const handleClick = async (e) => {
    if (!captureRegistrationSourceUrl) return;

    e.preventDefault();
    const csrfToken = document.querySelector('meta[name="csrf-token"]')?.content;
    try {
      await fetch(captureRegistrationSourceUrl, {
        method: "POST",
        headers: {
...
const EventParticipationLink = ({ participationUrl, btnText, captureRegistrationSourceUrl, style }) => {
  if (!participationUrl) return null;
...
const EventContentShow = ({ props }) => {
  const {
    ...
    track_activity_url, back_url, line_login_url, participation_url, capture_registration_source_url,
    add_friend_url,
