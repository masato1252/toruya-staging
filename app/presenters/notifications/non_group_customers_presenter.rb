  ...
  class NonGroupCustomersPresenter < ::NotificationsPresenter
    def data
      ...
      first_non_group_customer ? [
        "#{I18n.t("notifications.non_group_customers", number: recent_non_group_customers.count)} #{link_to(I18n.t("notifications.next_non_group_customer"), h.user_customers_path(first_non_group_customer.user, customer_id: first_non_group_customer.id))}"
      ] : []
    end
