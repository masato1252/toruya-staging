...
class Lines::UserBot::BookingPages::CustomMessagesController < Lines::UserBotDashboardController
  def index
    ...
    @booked_message = CustomMessage.scenario_of(@booking_page, CustomMessages::Template::BOOKING_PAGE_BOOKED).right_away.first
    @one_day_reminder_message = CustomMessage.scenario_of(@booking_page, CustomMessages::Template::BOOKING_PAGE_ONE_DAY_REMINDER).right_away.first
  end

  def edit_scenario
    @booking_page = super_user.booking_pages.find(params[:booking_page_id])
    @message = CustomMessage.find_by(service: @booking_page, id: params[:id])
    @template = @message ? @message.content : ::CustomMessages::Template.run!(product: @booking_page, scenario: params[:scenario])
  end
