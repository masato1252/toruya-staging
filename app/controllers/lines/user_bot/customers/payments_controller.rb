...
class Lines::UserBot::Customers::PaymentsController < Lines::UserBotDashboardController
  ...
  def refund
    ...
    if outcome.invalid?
      ...
      redirect_to lines_user_bot_customers_path(customer_id: customer_payment.customer_id, user_id: Current.business_owner.id, target_view: Customer::DASHBOARD_TARGET_VIEWS[:payments]), alert: outcome.errors.full_messages.to_sentence
    else
