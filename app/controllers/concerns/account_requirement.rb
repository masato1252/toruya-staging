module AccountRequirement
  ...
  def require_user_name
    if default_checking && current_user.name
  ...
  def require_contacts
    if default_checking && (!current_user.uid || !current_user.contact_groups.connected.exists? || current_user.contact_groups.unconnect.exists?)
  ...
  def require_staffs
    if default_checking && !current_user.staffs.exists?
  ...
  def require_working_times
    if default_checking && !BusinessSchedule.where(staff_id: current_user.staff_ids).exists?
  ...
  def require_reservation_settings
    if default_checking && !current_user.reservation_settings.exists?
  ...
  def default_checking
    !session[:account_requirement_checking_finished] && current_user
  end
end
