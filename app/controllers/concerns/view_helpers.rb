module ViewHelpers
  ...
  def super_user
    @super_user ||= User.find_by(id: params[:user_id]) || shop.try(:user)
  end

  def staffs_of_super_user
    if super_user && super_user != current_user && !super_user.owner_staffs_accounts.where(user_id: current_user.id).exists?
