class MembersController < DashboardController
  def show
    ...
    staffs_off_schedules = CustomSchedule.where(staff_id: staff.id).closed.where("start_time >= ? and end_time <= ?", @date.beginning_of_day, @date.end_of_day).includes(:staff).to_a
