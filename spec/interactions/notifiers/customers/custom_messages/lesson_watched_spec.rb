...
RSpec.describe Notifiers::Customers::CustomMessages::LessonWatched, :with_line do
  let(:receiver) { FactoryBot.create(:social_customer, customer: relation.customer).customer }
  let(:custom_message) { FactoryBot.create(:custom_message, service: lesson) }
  let(:lesson) { FactoryBot.create(:lesson) }
  let(:relation) { FactoryBot.create(:online_service_customer_relation, :paid, online_service: lesson.chapter.online_service) }
  let(:args) do
    {
      receiver: receiver,
