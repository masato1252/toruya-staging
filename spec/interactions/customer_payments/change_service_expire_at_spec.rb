...
RSpec.describe CustomerPayments::ChangeServiceExpireAt do
  let(:online_service_customer_relation) { FactoryBot.create(:online_service_customer_relation, :free, expire_at: 1.day.ago) }
  let(:expire_at) { nil }
  let(:args) do
    {
  ...
  describe "#execute" do
    ...
    context "when service is bundler" do
      ...
      let(:online_service_customer_relation) { FactoryBot.create(:online_service_customer_relation, :free, expire_at: 1.day.ago, online_service: bundler_service) }
      ...
      it "expires all relation under bundler service" do
        ...
        relation_with_end_at = FactoryBot.create(:online_service_customer_relation, :free, customer: customer, online_service: bundled_service_with_end_at.online_service, expire_at: Time.current.tomorrow)
        bundled_service_with_end_of_days = FactoryBot.create(:bundled_service, bundler_service: bundler_service, end_on_days: 3)
        relation_with_end_of_days = FactoryBot.create(:online_service_customer_relation, :free, customer: customer, online_service: bundled_service_with_end_of_days.online_service, expire_at: Time.current.advance(days: 3))
