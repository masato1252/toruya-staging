...
RSpec.describe Reservable::Menus do
  ...
  let(:menu) { FactoryBot.create(:menu, :normal, user: user, minutes: 60, shop: shop) }
  ...
  xdescribe "#run" do
    context "when staff is full time" do
      ...
      context "when menus reservation is available on each business days" do
        let!(:reservation_setting) { FactoryBot.create(:reservation_setting, day_type: "business_days", menu: menu) }
        ...
        context "when reservation setting time is not available" do
          let!(:reservation_setting) { FactoryBot.create(:reservation_setting, menu: menu, day_type: "business_days", start_time: now.advance(months: -1, minute: 0), end_time: now.advance(months: -1, minutes: 59)) }
        ...
        context "when menu does not have enough staffs" do
          let(:menu) { FactoryBot.create(:menu, user: user, minutes: 60, min_staffs_number: 2, max_seat_number: 6, shop: shop) }
        ...
        context "when staff does NOT have reservations" do
          ...
          context "when menu min_staffs_number > 1" do
            let(:menu) { FactoryBot.create(:menu, :lecture, user: user, minutes: 60, shop: shop) }
            ...
            context "when staffs count is more than menus.min_staffs_number" do
              ...
              context "when menu max_seat_number is more than customers number" do
                ...
                context "when staff's max_customers total is less than customers number" do
                  ...
                  let!(:menu) { FactoryBot.create(:menu, :lecture, user: user, minutes: 60, max_seat_number: customers_number + 1) }
        ...
        context "when staff already had reservations during that time" do
          ...
          context "when the existing reservation is the same shop" do
            ...
            context "when the existing reservation's menu min_staffs_number is 0" do
              let(:menu) { FactoryBot.create(:menu, :no_manpower, user: user, shop: shop) }
            ...
            context "when the existing reservations menu's needs coperation(min_staffs_number > 1)" do
              ...
              before do
                ...
                FactoryBot.create(:staff_menu, menu: menu, staff: staff2, max_customers: 4)
              end
      ...
      context "when menus reservation is available on each Friday" do
        ...
        let!(:reservation_setting) { FactoryBot.create(:reservation_setting, menu: menu, day_type: "weekly", days_of_week: [5]) }
      ...
      context "when menus reservation is available on second day of each Month" do
        ...
        let!(:reservation_setting) { FactoryBot.create(:reservation_setting, :number_of_day_monthly, menu: menu, day_type: "monthly", day: 2) }
      ...
      context "when menus reservation is available on second Friday of each Month" do
        ...
        let!(:reservation_setting) { FactoryBot.create(:reservation_setting, :day_of_week_monthly, menu: menu, day_type: "monthly", nth_of_week: 2, days_of_week: [5]) }
    ...
    context "when staff is full time but staff asks for leave on that date but not at that time" do
      before { FactoryBot.create(:staff_menu, menu: menu, staff: staff) }
      context "when menus reservation is available on each business days" do
        let!(:reservation_setting) { FactoryBot.create(:reservation_setting, menu: menu, day_type: "business_days") }
        let(:staff_max_customers) { staff.staff_menus.where(menu: menu).first.max_customers }
        before do
          FactoryBot.create(:custom_schedule, staff: staff, start_time: time_range.first.advance(hours: -2), end_time: time_range.last.advance(hours: -2))
        end
    ...
    context "when staff has work schedule on that date" do
      ...
      context "when menus reservation is available on each business days" do
        ...
        before do
          ...
          FactoryBot.create(:business_schedule, shop: shop, staff: staff, business_state: "opened",
